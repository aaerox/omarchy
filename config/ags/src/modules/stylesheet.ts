...
export class Stylesheet {
    ...
    #outputPath = Gio.File.new_for_path(`${GLib.get_user_cache_dir()}/omarchy-ags/style`);
    #stylesPaths: Array<string>;
    #gresourcePath = `${GLib.get_user_config_dir()}/ags/resources.gresource`;
    readonly #sassStyles = {
    ...
    private async compile(): Promise<void> {
        ...
        await execAsync(
            `bash -c "cd ${GLib.get_user_config_dir()}/ags && npx sass ${this.stylePath}/sass.scss ${this.stylePath}/style.css"`
        ).catch(_e => {
            const e = _e as Error;
    ...
    public compileApply(): void {
        this.compile().then(() => {
            import("../../app").then(({ OmarchyShell }) => {
                OmarchyShell.getDefault().resetStyle();
                OmarchyShell.getDefault().applyStyle(this.getStyleSheet());
            });
        }).catch(_e => {
            const e = _e as Error;
    ...
    constructor() {
        ...
        // Load our gresource
        const gresource = Gio.Resource.load(this.#gresourcePath);
        Gio.resources_register(gresource);

        this.#stylesPaths = Gio.resources_enumerate_children(
            "/io/omarchy/ags/styles", null
        ).map(name =>
            `/io/omarchy/ags/styles/${name}`
        );
        ...
        this.#stylesPaths.forEach(path => {
            ...
            switch(name) {
                ...
                break;
                case "wal":
                    // Skip wal file, we generate it from pywal
                break;
                case "mixins":
